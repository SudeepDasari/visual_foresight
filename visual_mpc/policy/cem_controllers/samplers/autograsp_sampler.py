...
class AutograspSampler(GaussianCEMSampler):
    ...
    def get_default_hparams():
        ...
        parent_params['action_norm_factor'] = 1.0
        parent_params['z_thresh'] = 0.15
        parent_params['gripper_close_cmd'] = 1
        parent_params['gripper_open_cmd'] = -1
        parent_params['resample_every_iter'] = False
        return parent_params
    
    def sample_initial_actions(self, t, nsamples, current_state):
        return self._sample_gripper(super(AutograspSampler, self).sample_initial_actions(t, nsamples, current_state))
    ...
    def sample_next_actions(self, n_samples, best_actions):
        ...
        if self._hp.resample_every_iter:
            return self._sample_gripper(default_actions)
        
        grip_act = np.zeros((nsamples, default_actions.shape[1], 1), dtype=np.float32)
        ...
        for t in range(default_actions.shape[1]):
            cmd_t = np.random.uniform(size=nsamples) < close_prob[t]
    ...
    def _sample_gripper(self, default_samples):
        import pdb; pdb.set_trace()
        grip_actions = np.zeros((nsamples, default_samples.shape[1], 1))

        for b in range(nsamples):
            close_mask = np.cumsum(default_samples[b, :, 2] * self._hp.action_norm_factor) + current_state[2] < self._hp.z_thresh
            ...
            grip_actions[b] = open_mask * self._hp.gripper_open_cmd + close_mask * self._hp.gripper_close_cmd
